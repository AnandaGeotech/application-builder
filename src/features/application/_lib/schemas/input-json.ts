...
const textInputSchema = z.object({
  ...
  length: z
    ...
    .refine(
      (length) => (length.min !== undefined && length.max !== undefined ? length.max > length.min : true),
      {
...
const selectInputSchema = z
  ...
  .refine((select) => (select.defaultValue !== undefined
    ? select.options.some((option) => option.value === select.defaultValue)
    : true), 'defaultValue must be a valid value from options');
